...
"""
翻译一下下面的函数：
把字典里的val和download取出来，如果val存在，就把val的路径转换成绝对路径，如果val不存在，就打印出来，如果download存在，就下载数据集，如果不存在，就抛出异常
"""
def check_dataset(dict):
    # Download dataset if not found locally
    val, s = dict.get('val'), dict.get('download')
    if val and len(val):
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # val path#isinstance(x,list)判断x是否是list类型
        if not all(x.exists() for x in val):
            print('\nWARNING: Dataset not found, nonexistent paths: %s' % [str(x) for x in val if not x.exists()])
            if s and len(s):  # download script
                print('Downloading %s ...' % s)
                if s.startswith('http') and s.endswith('.zip'):  # URL#endswith()判断字符串是否以指定的后缀结尾，startswith()判断字符串是否以指定的前缀开头
                    f = Path(s).name  # filename
                    torch.hub.download_url_to_file(s, f)#这是用来下载文件的函数,s是下载的地址，f是下载的文件名
                    r = os.system('unzip -q %s -d ../ && rm %s' % (f, f))  # unzip#这是用来解压文件的函数
                else:  # bash script
                    r = os.system(s)
